...
def parse_color(v: Optional[str]) -> Optional[discord.Color]:
    if not v:
        return None
    k = v.strip().lower().replace(" ", "_")
    if k in BRAND_COLORS:
        return discord.Color(BRAND_COLORS[k])
    if HEX_RE.match(k):
        if not k.startswith("#"):
            k = "#" + k
...
def ok_url(u: Optional[str]) -> bool:
...
def build_embed(
    ...
    col = parse_color(color) if color else discord.Color(BRAND_COLORS["midnight_blue"])
    emb = discord.Embed(title=title or discord.Embed.Empty,
                        description=description or discord.Embed.Empty,
                        color=col)
    if image_url and ok_url(image_url): emb.set_image(url=image_url)
    if thumbnail_url and ok_url(thumbnail_url): emb.set_thumbnail(url=thumbnail_url)
    if author_name:
        if author_icon and ok_url(author_icon):
    ...
    if footer:
        ...
        emb.set_footer(text="BotForge • Premium")
    return emb
...
class EmbedGroup(app_commands.Group):
    ...
    async def preview(self, interaction: discord.Interaction,
        ...
        try:
            emb = build_embed(title, description, color, image_url, thumbnail_url, footer, author_name, author_icon)
