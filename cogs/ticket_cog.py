...
class CloseView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.channel and interaction.channel.id == self.channel_id:
...
class TicketView(discord.ui.View):
    ...
    async def open_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        ch = await guild.create_text_channel(name=name[:90], category=category, overwrites=overwrites)
        await ch.send(
...
class TicketCog(commands.Cog):
    ...
    @app_commands.command(name="ticket_panel", description="Post the ticket button panel.")
